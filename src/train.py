"""Training entry point: reads config, trains a model, logs JSON, saves checkpoints."""
...
# Make sibling modules (model.py, dataset.py) importable whether we run from
# the repo root or from inside src/ (as happens in the Docker image).
...
def main():
    ...
    # Optional smoke-test shrink via env var, e.g. SUBSET_SIZE=200.
    subset_env = os.environ.get("SUBSET_SIZE")
    subset_size = int(subset_env) if subset_env else None
